...
class SSLCommerz
{
    ...
    public function __construct()
    {
        ...
        $this->store_id = "donat5c55c06bb4218";
        $this->store_pass = "donat5c55c06bb4218@ssl";
    ...
    public function orderValidate($trx_id = '', $amount = 0, $currency = "BDT", $post_data)
    {
        if ($post_data == '' && $trx_id == '' && !is_array($post_data)) {
            dd($this->error = "Please provide valid transaction ID and post request data");
            die();
            return $this->error;
        }
        ...
        if ($merchant_trans_id != "" && $merchant_trans_amount != 0) {
            ...
            if ($this->SSLCOMMERZ_hash_varify($this->store_pass))
            {
                ...
                if ($code == 200 && !(curl_errno($handle))) {
                    ...
                    if ($status == "VALID" || $status == "VALIDATED") {
                        if ($merchant_trans_currency == "BDT") {
                            if (trim($merchant_trans_id) == trim($tran_id) && (abs($merchant_trans_amount - $amount) < 1) && trim($merchant_trans_currency) == trim('BDT')) {
                                ...
                                # DATA TEMPERED
                                dd($this->error = "Data has been tempered");
                                die();
                                return false;
                            }
                        }
                        ...
                        # FAILED TRANSACTION
                        dd($this->error = "Failed Transaction");
                        die();
                        return false;
                    }
                }
                    ...
                    dd($this->error = "Faile to connect with SSLCOMMERZ");
                    die();
                    return false;
                }
            }
                ...
                dd($this->error = "Hash validation failed");
                die();
                return false;
            }
        } else {
            # INVALID DATA
            dd($this->error = "Invalid data");
            die();
            return false;
        }
    }
    ...
    public function SSLCOMMERZ_hash_varify($store_passwd = "")
    {
