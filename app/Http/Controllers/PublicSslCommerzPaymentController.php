...
class PublicSslCommerzPaymentController extends Controller
{
    ...
    public function index(Request $request)
    {
            ...
            $post_data = array();
            $post_data['store_id'] = "donat5c55c06bb4218";
            $post_data['store_passwd']  = "donat5c55c06bb4218@ssl";
            $post_data['total_amount'] = Session::get('transaction_data')['grand_total']; # You cant not pay less than 10
            $post_data['currency'] = Session::get('transaction_data')['currency'];
            $post_data['tran_id'] = Session::get('order_id'); // tran_id must be unique
            ...
            $post_data['cus_name'] = Session::get('transaction_data')['name'];
            $post_data['cus_email'] = Session::get('transaction_data')['email'];
            $post_data['cus_add1'] = Session::get('transaction_data')['address'];
            $post_data['cus_add2'] = "";
            $post_data['cus_city'] = Session::get('transaction_data')['city'];
            $post_data['cus_state'] = "";
            $post_data['cus_postcode'] = Session::get('transaction_data')['post_code'];
            $post_data['cus_country'] = Session::get('transaction_data')['country'];
            $post_data['cus_phone'] =Session::get('transaction_data')['phone_number'];
            $post_data['cus_fax'] = "";
            ...
            $total=Session::get('transaction_data')['grand_total'];

            #Before  going to initiate the payment order status need to update as Pending.
            $update_product = DB::table('orders')
                                    ->where('order_id', $post_data['tran_id'])
                                    ->update(['order_status' => 'Pending','currency' => $post_data['currency'],'grand_total' => $post_data['total_amount']]);
    ...
    public function success(Request $request)
    {
        ...
        if($order_detials->order_status=='Pending') {
            ...
            if ($validation == TRUE) {
                ...
                */
                ...
                return redirect('/transaction/success')->with('status', 'You have Successfully contributed For the development of Bangladesh and bangladeshi people. Thank you!');
            } else {
                /*
        ...
        else if($order_detials->order_status=='processing' || $order_detials->order_status=='completed')
        {
            /*
      ...
      if($request->input('tran_id')) #Check transation id is posted or not.
      {
        ...
        #Check order status in order tabel against the transaction id or order id.
         $order_details = DB::table('orders')
                ...
                else if($order_details->order_status == 'processing' || $order_details->order_status =='completed')
                {
